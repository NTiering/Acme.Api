...
namespace Acme.Data.Context
{
    public class DataContext : IDataContext
    {
        ...
        public async Task<T> GetModel<T>(Guid id)
                    where T : class, IDataModel
        {
        ...
        private async Task AddRecord<T>(T model) where T : class, IDataModel
        {
            model.Id = Guid.NewGuid();
            await _crudDataTools.AddModel(model);
        }

        private async Task ModifyRecord<T>(T original, T model) where T : class, IDataModel
        {
            await _crudDataTools.ModifyModel(original, model);
        }

        private async Task RemoveRecord<T>(Guid id) where T : class, IDataModel
        {
